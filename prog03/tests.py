from lu import lu_decomposition, det_from_lu

from numpy import ndarray as Matrix
import numpy as np


def test_lu_decomposition(A: Matrix):

    print(f"=== Testing matrix LU decomposition ===")
    ...
    assert np.allclose(np.triu(L), np.eye(*L.shape)), "L matrix is in invalid format."
    assert np.allclose(np.tril(U), np.diagflat(np.diag(U))), "U matrix is in invalid format."

    assert np.allclose(A, LU), "Results are inconsistent! Check your algorithm!"

    print("\nTest Passed!")

def test_det(A: Matrix):

    print(f"=== Testing matrix determinant from LU ===")
    ...
    print(f"\nMatrix A:\n{A}")
    print(f"\nDeterminant from LU:\n{det}")
    print(f"\nDeterminant from numpy module:\n{d}")

    assert np.isclose(det, d), "Results are inconsistent! Check your algorithm!"
...
if __name__ == "__main__":
    ...
    test_lu_decomposition(A1)
    test_lu_decomposition(A2)
    test_lu_decomposition(A3)
