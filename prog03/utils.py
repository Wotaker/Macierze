...
sum_flo = lambda k: pow(4, k)
str_flo = lambda k: 23/5 * pow(7, k) - 18 / 5 * pow(2, k)
inv_flo = lambda k: 17 * pow(2, k - 1) + sum([pow(2, k - i) * (46 * pow(7, i - 1) + 4 * pow(4, i - 1) - 36 * pow(2, i - 1) + 1) for i in range(2, k + 1)])
lu_flo = lambda k: pow(2, k) + sum([pow(2, k-i) * (2 * inv_flo(i-1) + 5 * str_flo(i-1) + sum_flo(i-1)) for i in range(1, k+1)])
...
def plot_results(sizes, times_mean, flos):
    ...
    ax1.set_title("Złożoność rekurencyjnej LU faktoryzacji")
